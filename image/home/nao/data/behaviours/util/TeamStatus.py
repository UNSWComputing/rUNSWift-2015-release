...
def minTeammateDistToBall():
   minDist = robot.FULL_FIELD_LENGTH
   for i in range(robot.ROBOTS_PER_TEAM):
      if isTeammateActive(i) and teammateLostBallCount(i) < 10 and \
         getTeammatePlayerNum(i) != myPlayerNumber():
         dist = getTimeToReachBall(i)
         if dist < minDist:
            minDist = dist
   return minDist
