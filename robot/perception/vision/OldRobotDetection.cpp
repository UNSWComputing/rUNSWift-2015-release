...
#define SC_HEADING_MERGE_THRESH     DEG2RAD(10.f)
...
// For spamming a massive information print out
      void debugInformation(bool shouldRemove,
         vector<RobotInfo>& robots, OldRobotDetection::RDData& robot, BBox& box, 
...
void OldRobotDetection::sanityCheckRobots(const Fovea &saliency) {
   ...
   for(unsigned int r = 0; r < allRobots.size(); ++r) {
      ...
      int dx, dy, mag;
      Point awayJersey = Point(0,0);
      Point homeJersey = Point(0,0);
      int awayJerseyCount = 0;
      int homeJerseyCount = 0;
      ...
      for(int x = box.a.x(); x < box.b.x(); ++x) {
         for(int y = box.a.y(); y < box.b.y(); ++y) {
            ...
            if(c == cTEAM_AWAY ){
               int awayJerseyColour = getColorAmount(saliency, y, x, cTEAM_AWAY, 1);
               if(awayJerseyColour  > awayJerseyCount){
            	  awayJerseyCount = awayJerseyColour;
            	  awayJersey = Point(x,y);
               }
            } else if (c == cTEAM_HOME) {
               int homeJerseyColour = getColorAmount(saliency, y, x, cTEAM_HOME, 1);
               if(homeJerseyColour > homeJerseyCount){
            	  homeJerseyCount = homeJerseyColour;
            	  homeJersey = Point(x,y);
               }
            }
      ...
      if(awayJerseyCount > homeJerseyCount && awayJerseyCount > SC_WAIST_BAND_ACCEPTABLE) {
         robot.robot.type = RobotInfo::rBlue;
         waistband = awayJersey;
         llog(DEBUG1) << "Waistband blue, ";
      } else if (homeJerseyCount > awayJerseyCount && homeJerseyCount > SC_WAIST_BAND_ACCEPTABLE) {
         robot.robot.type = RobotInfo::rRed;
         waistband = homeJersey;
         llog(DEBUG1) << "Waistband red, ";
      } else {
         robot.robot.type = RobotInfo::rUnknown;
       ...
       */
      bool feetSeen = true;
      if (saliency.top  &&  robot.robot.type != RobotInfo::rUnknown 
      ...
      if (!shouldRemove && (sonarVerified || (robot.robot.type != RobotInfo::rUnknown)) ){
...
void debugInformation(bool shouldRemove, vector<RobotInfo>& robots, OldRobotDetection::RDData& robot, 
