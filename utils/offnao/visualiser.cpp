...
Visualiser::Visualiser(QWidget *parent)
   ...
   reader(0), naoData(0), cb() {
      ...
      ipMap.insert(pair<string, int>("robot2", 11));
...
void Visualiser::connectToNao(const QString &naoName) {
	QString newnaoName = naoName;
	string strNao = naoName.toStdString();
	if (strNao.find('.') != string::npos){
		if (strNao[0] > '9'){
			strNao = strNao.substr(0, strNao.find('.'));

		}
	}
	if(ipMap.find(strNao) != ipMap.end()){
		int ip = ipMap[strNao];
		std::stringstream ss;
		ss << "192.168.18.1" << ip;
		newnaoName = ss.str().c_str();
	}
   if(reconnect<NetworkReader, pair<const QString &, OffNaoMask_t> >
      (make_pair(newnaoName, transmissionMask(NULL)))) {
