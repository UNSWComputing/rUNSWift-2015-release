...
class Wingman(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        destPos = self.destPos.minus(self.pos)
        dist = destPos.length()
        if dist <= DEST_THRESH :
            # Uatt = ATT_SCALE * (pos - destPos)
            Uatt = destPos.multiply(ATT_SCALE);
        else :
            # Uatt = ATT_SCALE * DEST_THRESH * (pos - destPos) / dist
            Uatt = destPos.multiply(ATT_SCALE * DEST_THRESH / dist)
        
        "repulsive field of each obstacle"
        Urep = Vector(0,0)
        for i in range(1, len(self.obstacles)) :
            pos = self.obstacles[i].pos.minus(self.pos)
            Urep = Urep.plus(self.repulsiveField(pos))

        "repulsive field of field edges"
#         Urep = Urep.plus(self.repulsiveField(Vector(self.pos.x, OFFNAO_FULL_FIELD_WIDTH/2)));
#         Urep = Urep.plus(self.repulsiveField(Vector(self.pos.x, -OFFNAO_FULL_FIELD_WIDTH/2)));
#         Urep = Urep.plus(self.repulsiveField(Vector(OFFNAO_FULL_FIELD_LENGTH/2, self.pos.y)));
#         Urep = Urep.plus(self.repulsiveField(Vector(-OFFNAO_FULL_FIELD_LENGTH/2, self.pos.y)));

        print (Uatt.toString(), Urep.toString())
        
        U = Uatt.plus(Urep)
        self.pos = self.pos.plus(U.multiply(8))
        screenCoords = convertFieldToScreenCoords(self.pos);
        self.rect.midtop = (screenCoords.x, screenCoords.y)
    ...
    def repulsiveField(self, obsPos):
        ...
        direction = obsPos.normalise()
        obsPos = direction.multiply(dist)
        if dist <= DIST_THRESH :
            #REP_SCALE * (1/DIST_THRESH - 1/dist)*(pos - obsPos)/dist^2
            Urep = obsPos.multiply(REP_SCALE * (1/DIST_THRESH - 1/dist)/(dist))
        return Urep
